...
export class AdminDashboardComponent implements OnInit {
  ...
  loadStats(): void {
    ...
    this.userService.getAllUsers().subscribe({
      next: (users) => {
        if (users && Array.isArray(users.data)) {
          ...
          this.stats.sinhVien = list.filter((u: { role: string; }) => u.role === 'Sinh viên').length;
          this.stats.giangVien = list.filter((u: { role: string; }) => u.role === 'Giảng viên').length;
          this.stats.quanTri = list.filter((u: { role: string; }) => u.role === 'Quản trị viên').length;
        }
      },
    ...
    // 3️⃣ Lấy danh sách lớp học phần
  ...
  updateChart(): void {
    this.chartData = {
      ...
      datasets: [
        {
          label: 'Số lượng',
          backgroundColor: ['#64b5f6', '#81c784', '#90caf9', '#ffb74d', '#ba68c8'],
          data: [
            this.stats.sinhVien,
